...
valor_Presente_Pension = function(x,sexo){
  ...
  costo_Pension_Anual <- function(r){
    return( (12 * axn( tablaAct, x = x, n = 1, m = r, k = 12) + axn(tablaAct, x = x, m = r, k = 1, payment = "immediate")) * (1 + inflacion) ^ r)
  }
...
valor_Presente_Pension_3 = function(x,sexo){
  ...
  anualidad_diferida = function(x,k){
    return(sum(Vectorize(pago)(x+k, 0:11),pago(x+k,11)) * pxt(tablaVida, x, k) * 
...
trabajadores_HOM <- trabajadores_HOM %>% 
  mutate( VP_PENSION =  (v_nom ^ TIEMPO_PARA_PENSION) * prob_65_Hom * vP_PENSION_TRAB_HOM * SALARIO_PENSION)

trabajadores_MUJ <- trabajadores_MUJ %>% 
  mutate( VP_PENSION =  (v_nom ^ TIEMPO_PARA_PENSION) * prob_65_Muj * vP_PENSION_TRAB_MUJ * SALARIO_PENSION)
...
#costo_Plan_Basico = calcular_Costo_Plan(tasa_interes_nominal, tasa_crec_salarios_nom)
